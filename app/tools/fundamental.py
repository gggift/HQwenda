...
@tool(
    ...
    description="全市场股票排名工具，可按总市值(total_mv)、流通市值(circ_mv)、市盈率(pe)、市净率(pb)、换手率(turnover_rate)、股息率(dv_ratio)等指标排名，返回前N名。适用于'市值最大的股票'、'换手率最高的股票'、'股息率最高的股票'等问题。",
    parameters={
        "type": "object",
        "properties": {
            "trade_date": {"type": "string", "description": "交易日期，格式 YYYYMMDD"},
            "sort_by": {"type": "string", "description": "排序字段：total_mv(总市值)、circ_mv(流通市值)、pe(市盈率)、pb(市净率)、turnover_rate(换手率)、dv_ratio(股息率)"},
...
def get_market_rank(trade_date: str, sort_by: str, ascending: bool = False, top_n: int = 10) -> dict:
    pro = _get_pro()
    valid_fields = {"total_mv", "circ_mv", "pe", "pb", "turnover_rate", "dv_ratio", "ps"}
    ...
    return {
        ...
        "note": "total_mv=总市值(亿元), circ_mv=流通市值(亿元), pe=市盈率, pb=市净率, turnover_rate=换手率(%), dv_ratio=股息率(%)",
    }
